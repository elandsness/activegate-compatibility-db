...
class HubExtensionsScraper:
    def __init__(
        self,
        base_url: str = "https://www.dynatrace.com/hub/?filter=all&type=extension&managed=true",
    ):
        ...
        try:
            ...
            for link in extension_links[:10]:
                ...
                ext_url = (
                    ...
                    else f"https://www.dynatrace.com{href}"
                )
                name = (
                    link.get_text().strip()
    ...
    def _scrape_extension_page(self, url: str) -> Dict:
        try:
            ...
            deployment_section = soup.find(
                string=lambda x: x and "deployment" in x.lower()
            )
