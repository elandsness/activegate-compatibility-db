...
def test_known_compatibility_cases():
    ...
    for case in KNOWN_CASES:
        ...
        result = reasoner.check_upgrade_compatibility(
            current_version=case["current"], target_version=case["target"]
        )
        ...
        if status_match:
            print(
                f"  ✓ Status: {result.status.value} (expected: {case['expected_status']})"
            )
            passed += 1
        else:
            print(
...
def test_extension_compatibility():
    ...
    result = reasoner.check_upgrade_compatibility(
        current_version="1.330", target_version="1.335", extensions=extensions
    )
...
def test_managed_cluster_compatibility():
    ...
    result = reasoner.check_upgrade_compatibility(
        current_version="1.330", target_version="1.335", managed_cluster_version="1.335"
    )

    print(f"  Status: {result.status.value}")
    print("  Managed Cluster: 1.335")
